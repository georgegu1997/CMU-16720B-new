...
def findLetters(image):
    ...
    image, n_label = skimage.measure.label(image, connectivity=2, return_num=True, background=0.0)

    # Get the region for each character
    bboxes = []
    for i in range(n_label+1):
        idx = np.array(np.asarray(image == i).nonzero())
        ...
        if (y2 - y1) < 25 or (y2 - y1) + (x2 - x1) < 20 or (y2 - y1) + (x2 - x1)>1000:
            continue
        bbox = [y1, x1, y2, x2]
        bboxes.append(bbox)
